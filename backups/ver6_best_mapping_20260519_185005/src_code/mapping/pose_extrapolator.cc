...
void PoseExtrapolator::LoadFusionConfigFromYaml() {
  ...
  curve_yaw_speed_threshold =
      ConfigValue(config, "curve_yaw_speed_threshold",
                  curve_yaw_speed_threshold);
  featureless_wheelodom_weight =
      ConfigValue(config, "featureless_wheelodom_weight",
                  featureless_wheelodom_weight);
  featureless_max_scan_to_odom_ratio =
      ConfigValue(config, "featureless_max_scan_to_odom_ratio",
                  featureless_max_scan_to_odom_ratio);
  featureless_imu_weight =
      ConfigValue(config, "featureless_imu_weight", featureless_imu_weight);
  ...
  LOG(INFO) << "Loaded pose extrapolator config from " << config_path
            ...
            << " featureless_imu_weight=" << featureless_imu_weight
...
void PoseExtrapolator::SetFeaturelessStraightMode(
    const bool enabled, const double degeneracy_ratio) {
  if (enabled != featureless_straight_mode) {
    LOG(INFO) << "Featureless straight mapping mode "
              << (enabled ? "enabled" : "disabled")
              << " degeneracy_ratio=" << degeneracy_ratio;
  }
  featureless_straight_mode = enabled;
  featureless_degeneracy_ratio = degeneracy_ratio;
}
...
Eigen::Vector3d PoseExtrapolator::translation_imu_wheel(const Eigen::Vector3d* linear_velocity_scan, const Eigen::Vector3d* linear_velocity_odom){
  ...
  if(velocity_from_scan_imu.norm() > 1e-6 && velocity_from_odom.norm() > 1e-6){
    Eigen::Vector2d scan_velocity = velocity_from_scan_imu;
    const double odom_speed = velocity_from_odom.norm();
    const double scan_speed = scan_velocity.norm();
    if (featureless_straight_mode && odom_speed > 1e-4 &&
        scan_speed > featureless_max_scan_to_odom_ratio * odom_speed) {
      scan_velocity *=
          (featureless_max_scan_to_odom_ratio * odom_speed / scan_speed);
    }

    const Eigen::Vector2d direction = scan_velocity.normalized();
    //현재 속도에 대한 단위 벡터를 구해 현재 속도에 대한 방향만 구한다
    const Eigen::Vector2d velocity_diff_from_odom = velocity_from_odom - scan_velocity;
    ...
    const double active_wheelodom_weight =
        featureless_straight_mode ? featureless_wheelodom_weight : wheelodom_weight;
    const Eigen::Vector2d velocity_from_wheel_fusion =
        velocity_diff_consider * active_wheelodom_weight;
    const Eigen::Vector2d linear_velocity_from_fusion =
        scan_velocity + velocity_from_wheel_fusion;
...
Eigen::Vector3d PoseExtrapolator::ExtrapolateTranslation(common::Time time) {
  ...
  if (fusion_extrpolator) {
    ...
    const double active_imu_weight =
        featureless_straight_mode ? featureless_imu_weight : imu_weight;
    const Eigen::Vector2d scan_velocity_with_imu =
        velocity_from_scan + active_imu_weight * imu_delta_velocity;
