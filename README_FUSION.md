...
# imu wheel odoemtry 활성화 -> true / 원본 로직 활성화 -> false
- 런치에서 켜기: `Damvi_carto*_launch.py`에 아래 두 줄이 있으면 됩니다.

```py
DeclareLaunchArgument('fusion_extrpolator', default_value='true')
SetEnvironmentVariable(name='FUSION_EXTRPOLATOR', value=fusion_extrpolator)
```

- 런치로 실행 시 값 넘기기 (예):

```bash
ros2 launch <pkg> Damvi_carto_pure_wheel_launch.py fusion_extrapolator:=false
```


직접 헤더 수정(간단)
- 파일: `src/cartographer/cartographer/mapping/pose_extrapolator.h`
- 변경 가능 항목:

	- `bool fusion_extrpolator = true;` — 기본값 직접 변경 가능 (예: `= false;`로 바꾸면 기본 동작이 off)
- 주의: 헤더를 직접 편집하면 재빌드해야 변경이 적용됩니다.

동작 요약 (fusion=true vs false)

- fusion = true:
	- `AddImuData()`에서 IMU 델타 속도 계산 및 IMU 기반 보정 분기 실행.
	- `AdvanceImuTracker()`는 pose-derived angular velocity를 우선 사용.
	- `ExtrapolateTranslation()` / `ExtrapolatePosesWithGravity()`는 scan(poses) + IMU(delta) + (가능하면) wheel-odom 보정 로직을 사용.
	- 호출되는 핵심 함수: `translation_fusion()`, `translation_imu_wheel()` 등 (자세한 구현: [src/cartographer/cartographer/mapping/pose_extrapolator.cc](src/cartographer/cartographer/mapping/pose_extrapolator.cc)).

- fusion = false:
	- `AddImuData()`는 기존 동작(간단한 enqueue 및 trim)만 수행.
	- `AdvanceImuTracker()`는 odometry가 있으면 odometry-derived angular velocity를 사용, 없으면 pose-derived를 사용.
	- `ExtrapolateTranslation()`는 pose- 또는 odometry-derived linear velocity만 사용(퓨전 로직 미사용).
